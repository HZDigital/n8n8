...
function sourceContext(state: CompletionContext['state'], startPos: number) {
	const pos = syntaxTree(state as unknown as Parameters<typeof syntaxTree>[0]).resolveInner(startPos, -1);
