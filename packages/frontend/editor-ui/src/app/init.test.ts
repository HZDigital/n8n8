	...
	describe('initializeCore()', () => {
		...
		it('should re-initialize ssoStore in login hook with authenticated settings', async () => {
			...
			const azureAd = {
				...
				loginUrl: 'http://localhost:5678/rest/azure-ad/login',
				ssoLoginUrl: 'http://localhost:5678/rest/azure-ad/login',
			};

			settingsStore.userManagement.authenticationMethod = UserManagementAuthenticationMethod.Oidc;
			settingsStore.settings.sso = { managedByEnv: false, saml, ldap, oidc, azureAd };
			...
			expect(ssoStore.initialize).toHaveBeenLastCalledWith({
				...
				managedByEnv: false,
				config: { managedByEnv: false, saml, ldap, oidc, azureAd },
				features: {
					saml: false,
					ldap: false,
					oidc: true,
		...
		it('should initialize ssoStore with settings SSO configuration', async () => {
			...
			expect(ssoStore.initialize).toHaveBeenCalledWith({
				...
				managedByEnv: false,
				config: { managedByEnv: false, saml, ldap, oidc, hideGenericSsoLoginButton: false, azureAd },
				features: {
					saml: true,
					ldap: false,
					oidc: false,
